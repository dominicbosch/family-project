...
def compile_http_sensor_getstat(IDeviceNum):
    sOutBuf = "http://192.168.0.79:8083/ZWaveAPI/Run/devices[" + str(IDeviceNum)
#    sOutBuf = sOutBuf + "].instances[0].commandClasses[0x84].Get()"
    sOutBuf = sOutBuf + "].instances[0].commandClasses[48].data.level"
    return sOutBuf
#http://192.168.0.79:8083/ZWaveAPI/Run/devices%5B5%5D.instances%5B0%5D.commandClasses%5B48%5D.Get()

def get_sensorstat(IDeviceNum):
    print("Refresh request : ", compile_httprefresh(IDeviceNum), "\n")
    resp, content = h.request(compile_httprefresh(IDeviceNum), "GET")
    print("Get Data request : ", compile_http_sensor_getstat(IDeviceNum), "\n")
    resp, content = h.request(compile_http_sensor_getstat(IDeviceNum), "GET")
    print("resp : ", resp, "\n")
...
def get_lighttoswitch():
    ...
    while bForEver == False:
        ...
        try:
            ...
            iInVal = 0
        if iInVal in range(2,5):
            return iInVal
        elif iInVal !=0:
            print("Currently only light switch 2 to 4 are active")
        else:
            return 0
...
while i != 0:
    ...
    print ("Current Switch full status : ", get_sensorstat(5)) 
