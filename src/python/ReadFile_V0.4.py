    ...
    for x in range(len(sInBuf)):
        if (sInBuf[x] [3] == iWeek) and (sInBuf[x] [4] == iDay):
            ...
            if sSwitchStat.find("Off") != -1:
                ...
                iSwitchStat = 1       
            sWorkBuf = [int(sWorkData[x] [0]), iSwitchStat, sWorkData[x] [7], sWorkData[x] [8], sWorkData[x] [9]]
...
#    if (sWorkData[x] [3] == iSelWeek) and (sWorkData[x] [4] == iSelDay):
#        sSwitchStat = sWorkData[x] [10]
#        if sSwitchStat.find("Off") != -1:
#            iSwitchStat = 0
#        else: 
#            iSwitchStat = 1       
#        sWorkBuf = [int(sWorkData[x] [0]), iSwitchStat, sWorkData[x] [7], sWorkData[x] [8], sWorkData[x] [9]]
#        sSelData.append(sWorkBuf)
